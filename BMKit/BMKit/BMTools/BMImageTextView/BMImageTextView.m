...
- (void)layoutSubviews
{
    ...
    if (self.imageName || self.imageUrl)
    {
        ...
        if (self.imageName)
        {
            ...
            {
                BMWeakSelf
                [self.imageView bmsd_setImageWithURL:[NSURL URLWithString:self.imageUrl] placeholderImage:placeholderImage options:BMSDWebImageRetryFailed|BMSDWebImageLowPriority completed:^(UIImage * _Nullable image, NSError * _Nullable error, BMSDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                    if (!error)
                    {
