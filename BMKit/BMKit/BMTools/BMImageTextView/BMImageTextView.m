...
- (void)layoutSubviews
{
    ...
    if (self.imageName || self.imageUrl)
    {
        ...
        if (self.imageName)
        {
            ...
            {
                BMWeakSelf
                [self.imageView bmsd_setImageWithURL:[NSURL URLWithString:self.imageUrl] host:nil placeholderImage:placeholderImage options:BMSDWebImageRetryFailed|BMSDWebImageLowPriority completed:^(UIImage * _Nullable image, NSError * _Nullable error, BMSDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                    if (!error)
                    {
    ...
    if (self.maxWidth)
    {
        ...
        if (self.maxWidth > imageWidth + 2*self.leftRightGap)
        {
            textMaxWidth = self.maxWidth - imageWidth - 2*self.leftRightGap;
    ...
    self.bm_width = imageWidth + textSize.width + 2*self.leftRightGap;
    ...
    self.arrowImageView.bm_left = self.bm_width - ARROWIMAGE_WIDTH - self.leftRightGap;
    if (self.showTableCellAccessoryArrow)
    {
    ...
    {
        self.imageView.bm_left = self.leftRightGap;
        self.textLabel.bm_left = self.leftRightGap + self.imageView.bm_width + self.imageTextGap;
    }
    else
    {
        self.textLabel.bm_left = self.leftRightGap;
        self.imageView.bm_left = self.leftRightGap + textSize.width + self.imageTextGap;
    }
