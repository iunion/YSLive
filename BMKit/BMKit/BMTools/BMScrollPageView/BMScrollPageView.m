...
- (void)moveScrollPageToIndex:(NSUInteger)index
{
    ...
    if ((NSNull *)view == [NSNull null])
    {
        ...
        if (newView)
        {
            ...
            BOOL needChangeTableSize = YES;
            if (self.datasource && [self.datasource respondsToSelector:@selector(scrollPageView:needChangeTableSizeAtIndex:)])
            {
                needChangeTableSize = [self.datasource scrollPageView:self needChangeTableSizeAtIndex:index];
            }
            if (needChangeTableSize)
            {
