...
- (void)cellWillAppear
{
    ...
    else
    {
        ...
        if (item.imageUrl)
        {
            [self.imageView bm_setImageWithURL:[NSURL URLWithString:item.imageUrl] placeholderImage:item.image options:SDWebImageRetryFailed|SDWebImageLowPriority completed:^(UIImage *image, NSError *error, BMSDImageCacheType cacheType, NSURL *imageURL) {
            }];
        }
        ...
        {
            BMWeakSelf
            [[BMSDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:item.highlightedImageUrl] options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL *targetURL) {
