...
#define BMDiskCacheError(error) if (error) { NSLog(@"%@ (%d) ERROR: %@", \
[[NSString stringWithUTF8String:__FILE__] lastPathComponent], \
__LINE__, [error localizedDescription]); }

#define BMDiskCacheException(exception) if (exception) { NSAssert(NO, [exception reason]); }

const char * BMDiskCacheAgeLimitAttributeName = "com.BMDiskCache.ageLimit";
NSString * const BMDiskCacheErrorDomain = @"com.BMDiskCache";
NSErrorUserInfoKey const BMDiskCacheErrorReadFailureCodeKey = @"BMDiskCacheErrorReadFailureCodeKey";
NSErrorUserInfoKey const BMDiskCacheErrorWriteFailureCodeKey = @"BMDiskCacheErrorWriteFailureCodeKey";
NSString * const BMDiskCachePrefix = @"com.BMDiskCache";
static NSString * const BMDiskCacheSharedName = @"BMDiskCacheShared";

static NSString * const BMDiskCacheOperationIdentifierTrimToDate = @"BMDiskCacheOperationIdentifierTrimToDate";
static NSString * const BMDiskCacheOperationIdentifierTrimToSize = @"BMDiskCacheOperationIdentifierTrimToSize";
static NSString * const BMDiskCacheOperationIdentifierTrimToSizeByDate = @"BMDiskCacheOperationIdentifierTrimToSizeByDate";

typedef NS_ENUM(NSUInteger, BMDiskCacheCondition)
{
    BMDiskCacheConditionNotReady = 0,
    BMDiskCacheConditionReady = 1,
};

static BMCOperationDataCoalescingBlock BMDiskTrimmingSizeCoalescingBlock = ^id(NSNumber *existingSize, NSNumber *newSize) {
    NSComparisonResult result = [existingSize compare:newSize];
    return (result == NSOrderedDescending) ? newSize : existingSize;
};

static BMCOperationDataCoalescingBlock BMDiskTrimmingDateCoalescingBlock = ^id(NSDate *existingDate, NSDate *newDate) {
    NSComparisonResult result = [existingDate compare:newDate];
    return (result == NSOrderedDescending) ? newDate : existingDate;
};

const char * BMDiskCacheFileSystemRepresentation(NSURL *url)
{
...
@interface BMDiskCacheMetadata : NSObject
...
@interface BMDiskCache ()
{
    BMDiskCacheSerializerBlock _serializer;
    BMDiskCacheDeserializerBlock _deserializer;
    
    BMDiskCacheKeyEncoderBlock _keyEncoder;
    BMDiskCacheKeyDecoderBlock _keyDecoder;
}
...
@property (strong, nonatomic) NSMutableDictionary <NSString *, BMDiskCacheMetadata *> *metadata;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    return [self initWithName:name rootPath:rootPath serializer:serializer deserializer:deserializer operationQueue:[BMCacheOperationQueue sharedOperationQueue]];
...
- (instancetype)initWithName:(NSString *)name
              ...
              operationQueue:(BMCacheOperationQueue *)operationQueue
{
                    ...
                    ttlCache:(BOOL)ttlCache
{
    ...
    NSAssert(((!serializer && !deserializer) || (serializer && deserializer)),
             @"BMDiskCache must be initialized with a serializer AND deserializer.");
    
    NSAssert(((!keyEncoder && !keyDecoder) || (keyEncoder && keyDecoder)),
             @"BMDiskCache must be initialized with an encoder AND decoder.");
    
    if (self = [super init]) {
        __unused int result = pthread_mutex_init(&_mutex, NULL);
        NSAssert(result == 0, @"Failed to init lock in BMDiskCache %@. Code: %d", self, result);
        ...
        _cacheFilePath = _cacheURL.absoluteString;
...
{
    return [[NSString alloc] initWithFormat:@"%@.%@.%p", BMDiskCachePrefix, _name, (__bridge void *)self];
}

+ (BMDiskCache *)sharedCache
{
    static BMDiskCache *cache;
    static dispatch_once_t predicate;
    
    dispatch_once(&predicate, ^{
        cache = [[BMDiskCache alloc] initWithName:BMDiskCacheSharedName];
    });
...
- (BMDiskCacheSerializerBlock)defaultSerializer
{
...
- (BMDiskCacheDeserializerBlock)defaultDeserializer
{
...
- (BMDiskCacheKeyEncoderBlock)defaultKeyEncoder
{
...
- (BMDiskCacheKeyEncoderBlock)defaultKeyDecoder
{
...
+ (dispatch_queue_t)sharedTrashQueue
{
    ...
    dispatch_once(&predicate, ^{
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.trash", BMDiskCachePrefix];
...
+ (void)emptyTrash
{
    dispatch_async([BMDiskCache sharedTrashQueue], ^{
        NSURL *trashURL = nil;
        ...
        if (trashURL != nil) {
            ...
            BMDiskCacheError(removeTrashedItemError);
        }
    });
}
...
- (NSUInteger)_locked_initializeDiskPropertiesForFile:(NSURL *)fileURL fileKey:(NSString *)fileKey
{
    ...
    NSDictionary *dictionary = [fileURL resourceValuesForKeys:[BMDiskCache resourceKeys] error:&error];
    BMDiskCacheError(error);
    
    if (_metadata[fileKey] == nil) {
        _metadata[fileKey] = [[BMDiskCacheMetadata alloc] init];
    }
    ...
    if (_ttlCache) {
        ...
        ssize_t res = getxattr(BMDiskCacheFileSystemRepresentation(fileURL), BMDiskCacheAgeLimitAttributeName, &ageLimit, sizeof(NSTimeInterval), 0, 0);
        if(res > 0) {
            _metadata[fileKey].ageLimit = ageLimit;
        } else if (res == -1) {
            // Ignore if the extended attribute was never recorded for this file.
            if (errno != ENOATTR) {
                NSDictionary<NSErrorUserInfoKey, id> *userInfo = @{ BMDiskCacheErrorReadFailureCodeKey : @(errno)};
                error = [NSError errorWithDomain:BMDiskCacheErrorDomain code:BMDiskCacheErrorReadFailure userInfo:userInfo];
                BMDiskCacheError(error);
            }
        }
    }
...
- (void)initializeDiskProperties
{
    ...
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtURL:_cacheURL
                                                   includingPropertiesForKeys:[BMDiskCache resourceKeys]
...
- (BOOL)_locked_setAgeLimit:(NSTimeInterval)ageLimit forURL:(NSURL *)fileURL
{
    ...
    NSError *error = nil;
    if (ageLimit <= 0.0) {
        if (removexattr(BMDiskCacheFileSystemRepresentation(fileURL), BMDiskCacheAgeLimitAttributeName, 0) != 0) {
            // Ignore if the extended attribute was never recorded for this file.
            if (errno != ENOATTR) {
                NSDictionary<NSErrorUserInfoKey, id> *userInfo = @{ BMDiskCacheErrorWriteFailureCodeKey : @(errno)};
                error = [NSError errorWithDomain:BMDiskCacheErrorDomain code:BMDiskCacheErrorWriteFailure userInfo:userInfo];
                BMDiskCacheError(error);
            }
        }
    } else {
        if (setxattr(BMDiskCacheFileSystemRepresentation(fileURL), BMDiskCacheAgeLimitAttributeName, &ageLimit, sizeof(NSTimeInterval), 0, 0) != 0) {
            NSDictionary<NSErrorUserInfoKey, id> *userInfo = @{ BMDiskCacheErrorWriteFailureCodeKey : @(errno)};
            error = [NSError errorWithDomain:BMDiskCacheErrorDomain code:BMDiskCacheErrorWriteFailure userInfo:userInfo];
            BMDiskCacheError(error);
        }
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    BMDiskCacheObjectBlock willRemoveObjectBlock = _willRemoveObjectBlock;
    ...
    BMDiskCacheObjectBlock didRemoveObjectBlock = _didRemoveObjectBlock;
...
- (void)trimDiskToSize:(NSUInteger)trimByteCount
{
    ...
    if (_byteCount > trimByteCount) {
        ...
        NSArray *keysSortedBySize = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(BMDiskCacheMetadata * _Nonnull obj1, BMDiskCacheMetadata * _Nonnull obj2) {
...
- (void)trimDiskToSizeByDate:(NSUInteger)trimByteCount
{
    ...
    if (_byteCount > trimByteCount) {
        ...
        NSArray *keysSortedByLastModifiedDate = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(BMDiskCacheMetadata * _Nonnull obj1, BMDiskCacheMetadata * _Nonnull obj2) {
...
{
    [self lockForWriting];
    NSArray *keysSortedByCreatedDate = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(BMDiskCacheMetadata * _Nonnull obj1, BMDiskCacheMetadata * _Nonnull obj2) {
...
- (void)containsObjectForKeyAsync:(NSString *)key completion:(BMDiskCacheContainsBlock)block
{
    if (!key || !block)
        return;
...
- (void)objectForKeyAsync:(NSString *)key completion:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)setObjectAsync:(id <NSCoding>)object forKey:(NSString *)key completion:(BMDiskCacheObjectBlock)block
{
    [self setObjectAsync:object forKey:key withAgeLimit:0.0 completion:(BMDiskCacheObjectBlock)block];
}
...
{
    [self setObjectAsync:object forKey:key completion:(BMDiskCacheObjectBlock)block];
}
...
{
    [self setObjectAsync:object forKey:key withAgeLimit:ageLimit completion:(BMDiskCacheObjectBlock)block];
}

- (void)removeObjectForKeyAsync:(NSString *)key completion:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)trimToSizeAsync:(NSUInteger)trimByteCount completion:(BMCacheBlock)block
{
    BMCOperationBlock operation = ^(id data) {
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:BMCOperationQueuePriorityLow
                                identifier:BMDiskCacheOperationIdentifierTrimToSize
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:BMDiskTrimmingSizeCoalescingBlock
...
- (void)trimToDateAsync:(NSDate *)trimDate completion:(BMCacheBlock)block
{
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:BMCOperationQueuePriorityLow
                                identifier:BMDiskCacheOperationIdentifierTrimToDate
                            coalescingData:trimDate
                       dataCoalescingBlock:BMDiskTrimmingDateCoalescingBlock
                                completion:completion];
}

- (void)trimToSizeByDateAsync:(NSUInteger)trimByteCount completion:(BMCacheBlock)block
{
    BMCOperationBlock operation = ^(id data){
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:BMCOperationQueuePriorityLow
                                identifier:BMDiskCacheOperationIdentifierTrimToSizeByDate
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:BMDiskTrimmingSizeCoalescingBlock
                                completion:completion];
}
...
- (void)enumerateObjectsWithBlockAsync:(BMDiskCacheFileURLEnumerationBlock)block completionBlock:(BMCacheBlock)completionBlock
{
    [self.operationQueue scheduleOperation:^{
    ...
    if (!self->_ttlCache || ageLimit <= 0 || fabs([_metadata[key].createdDate timeIntervalSinceDate:now]) < ageLimit) {
        ...
        if (objectData) {
            ...
            @catch (NSException *exception) {
                ...
                [self unlock];
                BMDiskCacheError(error)
                BMDiskCacheException(exception);
            }
            [self lock];
        }
        if (object) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key withAgeLimit:(NSTimeInterval)ageLimit fileURL:(NSURL **)outFileURL
{
    ...
    [self lockForWriting];
    BMDiskCacheObjectBlock willAddObjectBlock = self->_willAddObjectBlock;
    ...
    BMDiskCacheError(writeError);
    
    if (written) {
        if (_metadata[key] == nil) {
            _metadata[key] = [[BMDiskCacheMetadata alloc] init];
        }
    ...
    BMDiskCacheObjectBlock didAddObjectBlock = self->_didAddObjectBlock;
...
- (void)removeExpiredObjects
{
    ...
    [_metadata enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, BMDiskCacheMetadata * _Nonnull obj, BOOL * _Nonnull stop) {
...
- (void)removeAllObjects
{
    ...
    [self lockForWriting];
    BMCacheBlock willRemoveAllObjectsBlock = self->_willRemoveAllObjectsBlock;
    ...
    BMCacheBlock didRemoveAllObjectsBlock = self->_didRemoveAllObjectsBlock;
...
- (BMDiskCacheObjectBlock)willAddObjectBlock
{
    BMDiskCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (BMDiskCacheObjectBlock)willRemoveObjectBlock
{
    BMDiskCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (BMDiskCacheObjectBlock)didAddObjectBlock
{
    BMDiskCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (BMDiskCacheObjectBlock)didRemoveObjectBlock
{
    BMDiskCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(BMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
#if 0
@implementation BMDiskCache (Deprecated)

- (void)lockFileAccessWhileExecutingBlock:(nullable BMCacheBlock)block
{
    [self lockFileAccessWhileExecutingBlockAsync:block];
}

- (void)containsObjectForKey:(NSString *)key block:(BMDiskCacheContainsBlock)block
{
    [self containsObjectForKeyAsync:key completion:block];
}

- (void)objectForKey:(NSString *)key block:(nullable BMDiskCacheObjectBlock)block
{
    [self objectForKeyAsync:key completion:block];
}

- (void)fileURLForKey:(NSString *)key block:(nullable BMDiskCacheFileURLBlock)block
{
    [self fileURLForKeyAsync:key completion:block];
}

- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(nullable BMDiskCacheObjectBlock)block
{
    [self setObjectAsync:object forKey:key completion:block];
}

- (void)removeObjectForKey:(NSString *)key block:(nullable BMDiskCacheObjectBlock)block
{
    [self removeObjectForKeyAsync:key completion:block];
}

- (void)trimToDate:(NSDate *)date block:(nullable BMDiskCacheBlock)block
{
    [self trimToDateAsync:date completion:^(id<BMCacheProtocol> diskCache) {
        if (block) {
            block((BMDiskCache *)diskCache);
        }
    }];
}

- (void)trimToSize:(NSUInteger)byteCount block:(nullable BMDiskCacheBlock)block
{
    [self trimToSizeAsync:byteCount completion:^(id<BMCacheProtocol> diskCache) {
        if (block) {
            block((BMDiskCache *)diskCache);
        }
    }];
...
- (void)trimToSizeByDate:(NSUInteger)byteCount block:(nullable BMDiskCacheBlock)block
{
    [self trimToSizeAsync:byteCount completion:^(id<BMCacheProtocol> diskCache) {
        if (block) {
            block((BMDiskCache *)diskCache);
...
- (void)removeAllObjects:(nullable BMDiskCacheBlock)block
{
    [self removeAllObjectsAsync:^(id<BMCacheProtocol> diskCache) {
        if (block) {
            block((BMDiskCache *)diskCache);
        }
    }];
...
- (void)enumerateObjectsWithBlock:(BMDiskCacheFileURLBlock)block completionBlock:(nullable BMDiskCacheBlock)completionBlock
{
    [self enumerateObjectsWithBlockAsync:^(NSString * _Nonnull key, NSURL * _Nullable fileURL, BOOL * _Nonnull stop) {
        block(key, fileURL);
    } completionBlock:^(id<BMCacheProtocol> diskCache) {
        if (completionBlock) {
            completionBlock((BMDiskCache *)diskCache);
        }
    }];
}
...
@implementation BMDiskCacheMetadata
@end
