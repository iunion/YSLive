...
inline UIImage *BMSDScaledImageForKey(NSString * _Nullable key, UIImage * _Nullable image) {
    if (!image) {
        return nil;
    }
    ...
    if ((image.images).count > 0) {
        ...
        for (UIImage *tempImage in image.images) {
            [scaledImages addObject:BMSDScaledImageForKey(key, tempImage)];
        }
...
NSString *const BMSDWebImageErrorDomain = @"SDWebImageErrorDomain";
