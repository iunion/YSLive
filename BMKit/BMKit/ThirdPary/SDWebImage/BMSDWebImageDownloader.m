...
- (void)setOperationClass:(nullable Class)operationClass {
    if (operationClass && [operationClass isSubclassOfClass:[NSOperation class]] && [operationClass conformsToProtocol:@protocol(BMSDWebImageDownloaderOperationInterface)]) {
...
- (nullable BMSDWebImageDownloadToken *)downloadImageWithURL:(nullable NSURL *)url
    ...
    return [self addProgressCallback:progressBlock completedBlock:completedBlock forURL:url createCallback:^BMSDWebImageDownloaderOperation *{
        ...
        BMSDWebImageDownloaderOperation *operation = [[sself.operationClass alloc] initWithRequest:request inSession:sself.session options:options];
...
- (void)cancel:(nullable BMSDWebImageDownloadToken *)token {
    dispatch_barrier_async(self.barrierQueue, ^{
        BMSDWebImageDownloaderOperation *operation = self.URLOperations[token.url];
...
- (nullable BMSDWebImageDownloadToken *)addProgressCallback:(BMSDWebImageDownloaderProgressBlock)progressBlock
    ...
    __block BMSDWebImageDownloadToken *token = nil;

    dispatch_barrier_sync(self.barrierQueue, ^{
        BMSDWebImageDownloaderOperation *operation = self.URLOperations[url];
        if (!operation) {
            operation = createCallback();
            self.URLOperations[url] = operation;

            __weak BMSDWebImageDownloaderOperation *woperation = operation;
            operation.completionBlock = ^{
				dispatch_barrier_sync(self.barrierQueue, ^{
					BMSDWebImageDownloaderOperation *soperation = woperation;
        ...
        token = [BMSDWebImageDownloadToken new];
        token.url = url;
...
- (BMSDWebImageDownloaderOperation *)operationWithTask:(NSURLSessionTask *)task {
    BMSDWebImageDownloaderOperation *returnOperation = nil;
    for (BMSDWebImageDownloaderOperation *operation in self.downloadQueue.operations) {
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    BMSDWebImageDownloaderOperation *dataOperation = [self operationWithTask:dataTask];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    // Identify the operation that runs this task and pass it the delegate method
    BMSDWebImageDownloaderOperation *dataOperation = [self operationWithTask:task];
