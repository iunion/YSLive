...
@property (nonatomic, strong) id<BMSDAnimatedImageProvider> animatedProvider;
...
@implementation BMSDAnimatedImagePlayer

- (instancetype)initWithProvider:(id<BMSDAnimatedImageProvider>)provider {
    self = [super init];
    if (self) {
...
+ (instancetype)playerWithProvider:(id<BMSDAnimatedImageProvider>)provider {
    BMSDAnimatedImagePlayer *player = [[BMSDAnimatedImagePlayer alloc] initWithProvider:provider];
    return player;
}
...
- (BMSDDisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [BMSDDisplayLink displayLinkWithTarget:self selector:@selector(displayDidRefresh:)];
...
- (void)displayDidRefresh:(BMSDDisplayLink *)displayLink {
    ...
    if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
        ...
        id<BMSDAnimatedImageProvider> animatedProvider = self.animatedProvider;
        @bmweakify(self);
...
- (void)calculateMaxBufferCount {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = [BMSDDeviceHelper totalMemory];
        NSUInteger free = [BMSDDeviceHelper freeMemory];
