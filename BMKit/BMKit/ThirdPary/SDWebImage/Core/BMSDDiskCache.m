...
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull BMSDImageCacheConfig *)config {
    if (self = [super init]) {
...
- (NSData *)extendedDataForKey:(NSString *)key {
    ...
    NSData *extendedData = [BMSDFileAttributeHelper extendedAttribute:BMSDDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
...
- (void)setExtendedData:(NSData *)extendedData forKey:(NSString *)key {
    ...
    if (!extendedData) {
        // Remove
        [BMSDFileAttributeHelper removeExtendedAttribute:BMSDDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
    } else {
        // Override
        [BMSDFileAttributeHelper setExtendedAttribute:BMSDDiskCacheExtendedAttributeName value:extendedData atPath:cachePathForKey traverseLink:NO overwrite:YES error:nil];
    }
}
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case BMSDImageCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
        case BMSDImageCacheConfigExpireTypeModificationDate:
            cacheContentDateKey = NSURLContentModificationDateKey;
            break;
        case BMSDImageCacheConfigExpireTypeCreationDate:
            cacheContentDateKey = NSURLCreationDateKey;
            break;
        case BMSDImageCacheConfigExpireTypeChangeDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = BMSDDiskCacheFileNameForKey(key);
...
#define BMSD_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull BMSDDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    if (ext.length > BMSD_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
