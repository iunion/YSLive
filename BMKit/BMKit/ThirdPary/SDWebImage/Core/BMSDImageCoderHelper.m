...
static inline size_t BMSDByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

static const size_t kBMBytesPerPixel = 4;
 ...
 */
#if BMSD_MAC
static CGFloat kBMDestImageLimitBytes = 90.f * kBMBytesPerMB;
#elif BMSD_UIKIT
static CGFloat kBMDestImageLimitBytes = 60.f * kBMBytesPerMB;
#elif BMSD_WATCH
static CGFloat kBMDestImageLimitBytes = 30.f * kBMBytesPerMB;
#endif

static const CGFloat kBMDestSeemOverlap = 2.0f;   // the numbers of pixels to overlap the seems where tiles meet.

@implementation BMSDImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<BMSDImageFrame *> *)frames {
    ...
    NSUInteger const gcd = bm_gcdArray(frameCount, durations);
    __block NSUInteger totalDuration = 0;
    NSMutableArray<UIImage *> *animatedImages = [NSMutableArray arrayWithCapacity:frameCount];
    [frames enumerateObjectsUsingBlock:^(BMSDImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (NSArray<BMSDImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            BMSDImageFrame *frame = [BMSDImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
    ...
    output_buffer.rowBytes = BMSDByteAlign(output_buffer.width * 4, 64);
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    BMSDImageCopyAssociatedObject(image, decodedImage);
    decodedImage.bmsd_isDecoded = YES;
    return decodedImage;
}
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    @autoreleasepool {
        ...
        float sourceSeemOverlap = (int)((kBMDestSeemOverlap/destResolution.height)*sourceResolution.height);
        ...
        destTile.size.height += kBMDestSeemOverlap;
        for( int y = 0; y < iterations; ++y ) {
            @autoreleasepool {
                sourceTile.origin.y = y * sourceTileHeightMinusOverlap + sourceSeemOverlap;
                destTile.origin.y = destResolution.height - (( y + 1 ) * sourceTileHeightMinusOverlap * imageScale + kBMDestSeemOverlap);
        ...
        BMSDImageCopyAssociatedObject(image, destImage);
        destImage.bmsd_isDecoded = YES;
        return destImage;
    }
}

+ (NSUInteger)defaultScaleDownLimitBytes {
    return kBMDestImageLimitBytes;
}
...
#if BMSD_UIKIT || BMSD_WATCH
static NSUInteger bm_gcd(NSUInteger a, NSUInteger b) {
    NSUInteger c;
    while (a != 0) {
        c = a;
        a = b % a;
...
static NSUInteger bm_gcdArray(size_t const count, NSUInteger const * const values) {
    if (count == 0) {
        return 0;
    }
    NSUInteger result = values[0];
    for (size_t i = 1; i < count; ++i) {
        result = bm_gcd(values[i], result);
    }
    return result;
}
#endif
