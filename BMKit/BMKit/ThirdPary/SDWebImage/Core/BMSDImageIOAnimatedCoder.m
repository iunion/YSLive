...
static NSString * kBMSDCGImageDestinationRequestedFileSize = @"kCGImageDestinationRequestedFileSize";

@interface BMSDImageIOCoderFrame : NSObject
...
@implementation BMSDImageIOAnimatedCoder {
    ...
    NSArray<BMSDImageIOCoderFrame *> *_frames;
    BOOL _finished;
...
+ (BOOL)canDecodeFromFormat:(BMSDImageFormat)format {
    ...
    CFStringRef imageUTType = [NSData bmsd_UTTypeFromImageFormat:format];
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize options:(NSDictionary *)options {
    ...
    // Check vector format
    BOOL isVector = NO;
    if ([NSData bmsd_imageFormatFromUTType:uttype] == BMSDImageFormatPDF) {
        isVector = YES;
    }
    ...
    if (createFullImage) {
        if (isVector) {
            ...
            decodingOptions[kBMSDCGImageSourceRasterizationDPI] = @(rasterizationDPI);
        }
    ...
    if (!createFullImage) {
        if (preserveAspectRatio) {
            ...
            CGImageRef scaledImageRef = [BMSDImageCoderHelper CGImageCreateScaled:imageRef size:thumbnailSize];
    ...
    UIImageOrientation imageOrientation = [BMSDImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData bmsd_imageFormatForImageData:data] == self.class.imageFormat);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable BMSDImageCoderOptions *)options {
    ...
    CGFloat scale = 1;
    NSNumber *scaleFactor = options[BMSDImageCoderDecodeScaleFactor];
    if (scaleFactor != nil) {
    ...
    NSNumber *preserveAspectRatioValue = options[BMSDImageCoderDecodePreserveAspectRatio];
    ...
    BOOL decodeFirstFrame = [options[BMSDImageCoderDecodeFirstFrameOnly] boolValue];
    if (decodeFirstFrame || count <= 1) {
        ...
        NSMutableArray<BMSDImageFrame *> *frames = [NSMutableArray array];
        ...
        animatedImage = [BMSDImageCoderHelper animatedImageWithFrames:frames];
        animatedImage.bmsd_imageLoopCount = loopCount;
    }
    animatedImage.bmsd_imageFormat = self.class.imageFormat;
    CFRelease(source);
...
- (instancetype)initIncrementalWithOptions:(nullable BMSDImageCoderOptions *)options {
    self = [super init];
    if (self) {
...
- (UIImage *)incrementalDecodedImageWithOptions:(BMSDImageCoderOptions *)options {
    UIImage *image;
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(BMSDImageFormat)format options:(nullable BMSDImageCoderOptions *)options {
    ...
    CFStringRef imageUTType = [NSData bmsd_UTTypeFromImageFormat:format];
    NSArray<BMSDImageFrame *> *frames = [BMSDImageCoderHelper framesFromAnimatedImage:image];
    ...
    if (options[BMSDImageCoderEncodeCompressionQuality]) {
        compressionQuality = [options[BMSDImageCoderEncodeCompressionQuality] doubleValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationLossyCompressionQuality] = @(compressionQuality);
    CGColorRef backgroundColor = [options[BMSDImageCoderEncodeBackgroundColor] CGColor];
    if (backgroundColor) {
        properties[(__bridge NSString *)kCGImageDestinationBackgroundColor] = (__bridge id)(backgroundColor);
    }
    CGSize maxPixelSize = CGSizeZero;
    NSValue *maxPixelSizeValue = options[BMSDImageCoderEncodeMaxPixelSize];
    ...
    NSUInteger maxFileSize = [options[BMSDImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    if (maxFileSize > 0) {
        properties[kBMSDCGImageDestinationRequestedFileSize] = @(maxFileSize);
    ...
    if (options[BMSDImageCoderEncodeEmbedThumbnail]) {
        embedThumbnail = [options[BMSDImageCoderEncodeEmbedThumbnail] boolValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationEmbedThumbnail] = @(embedThumbnail);
    
    BOOL encodeFirstFrame = [options[BMSDImageCoderEncodeFirstFrameOnly] boolValue];
    if (encodeFirstFrame || frames.count == 0) {
        // for static single images
        CGImageDestinationAddImage(imageDestination, imageRef, (__bridge CFDictionaryRef)properties);
    } else {
        // for animated images
        NSUInteger loopCount = image.bmsd_imageLoopCount;
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<BMSDImageIOCoderFrame *> *frames = [NSMutableArray array];
    
    for (size_t i = 0; i < frameCount; i++) {
        BMSDImageIOCoderFrame *frame = [[BMSDImageIOCoderFrame alloc] init];
        frame.index = i;
