...
#import "BMSDWebImagePrefetcher.h"
#import "BMSDAsyncBlockOperation.h"
#import "BMSDInternalMacros.h"
#import <stdatomic.h>

@interface BMSDWebImagePrefetchToken () {
    @public
    ...
    BMSD_LOCK_DECLARE(_prefetchOperationsLock);
    BMSD_LOCK_DECLARE(_loadOperationsLock);
}
...
@property (nonatomic, weak) BMSDWebImagePrefetcher *prefetcher;
@property (nonatomic, copy, nullable) BMSDWebImagePrefetcherCompletionBlock completionBlock;
@property (nonatomic, copy, nullable) BMSDWebImagePrefetcherProgressBlock progressBlock;

@end

@interface BMSDWebImagePrefetcher ()

@property (strong, nonatomic, nonnull) BMSDWebImageManager *manager;
@property (strong, atomic, nonnull) NSMutableSet<BMSDWebImagePrefetchToken *> *runningTokens;
@property (strong, nonatomic, nonnull) NSOperationQueue *prefetchQueue;

@end

@implementation BMSDWebImagePrefetcher
...
- (nonnull instancetype)init {
    return [self initWithImageManager:[BMSDWebImageManager new]];
}
...
- (nullable BMSDWebImagePrefetchToken *)prefetchURLs:(nullable NSArray<NSURL *> *)urls
    ...
    BMSDWebImagePrefetchToken *token = [BMSDWebImagePrefetchToken new];
...
- (void)startPrefetchWithToken:(BMSDWebImagePrefetchToken * _Nonnull)token {
    for (NSURL *url in token.urls) {
        @autoreleasepool {
            @bmweakify(self);
            BMSDAsyncBlockOperation *prefetchOperation = [BMSDAsyncBlockOperation blockOperationWithBlock:^(BMSDAsyncBlockOperation * _Nonnull asyncOperation) {
                @bmstrongify(self);
                ...
                BMSD_LOCK(token->_loadOperationsLock);
                [token.loadOperations addPointer:(__bridge void *)operation];
                BMSD_UNLOCK(token->_loadOperationsLock);
            }];
            BMSD_LOCK(token->_prefetchOperationsLock);
            [token.prefetchOperations addPointer:(__bridge void *)prefetchOperation];
            BMSD_UNLOCK(token->_prefetchOperationsLock);
...
- (void)cancelPrefetching {
    @synchronized(self.runningTokens) {
        NSSet<BMSDWebImagePrefetchToken *> *copiedTokens = [self.runningTokens copy];
        [copiedTokens makeObjectsPerformSelector:@selector(cancel)];
        [self.runningTokens removeAllObjects];
    }
}

- (void)callProgressBlockForToken:(BMSDWebImagePrefetchToken *)token imageURL:(NSURL *)url {
    if (!token) {
        return;
    }
...
- (void)callCompletionBlockForToken:(BMSDWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
...
- (NSUInteger)tokenTotalCount {
    ...
    @synchronized (self.runningTokens) {
        for (BMSDWebImagePrefetchToken *token in self.runningTokens) {
...
- (void)addRunningToken:(BMSDWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
    @synchronized (self.runningTokens) {
...
- (void)removeRunningToken:(BMSDWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
    @synchronized (self.runningTokens) {
...
@implementation BMSDWebImagePrefetchToken

- (instancetype)init {
    self = [super init];
    if (self) {
        BMSD_LOCK_INIT(_prefetchOperationsLock);
        BMSD_LOCK_INIT(_loadOperationsLock);
    }
    return self;
}

- (void)cancel {
    BMSD_LOCK(_prefetchOperationsLock);
    [self.prefetchOperations compact];
    for (id operation in self.prefetchOperations) {
        id<BMSDWebImageOperation> strongOperation = operation;
        if (strongOperation) {
            [strongOperation cancel];
        }
    }
    self.prefetchOperations.count = 0;
    BMSD_UNLOCK(_prefetchOperationsLock);
    
    BMSD_LOCK(_loadOperationsLock);
    ...
    BMSD_UNLOCK(_loadOperationsLock);
