...
static inline CGRect BMSDCGRectFitWithScaleMode(CGRect rect, CGSize size, BMSDImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case BMSDImageScaleModeAspectFit:
        case BMSDImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == BMSDImageScaleModeAspectFit) {
...
static inline UIColor * BMSDGetColorFromPixel(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
...
static inline CGImageRef _Nullable BMSDCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
@implementation UIImage (BMTransform)

- (void)bmsd_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(BMSDImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = BMSDCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)bmsd_resizedImageWithSize:(CGSize)size scaleMode:(BMSDImageScaleMode)scaleMode {
    ...
    BMSDGraphicsImageRendererFormat *format = [[BMSDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    BMSDGraphicsImageRenderer *renderer = [[BMSDGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self bmsd_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}
...
- (nullable UIImage *)bmsd_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(BMSDRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    BMSDGraphicsImageRendererFormat *format = [[BMSDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    BMSDGraphicsImageRenderer *renderer = [[BMSDGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIImage *)bmsd_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    BMSDGraphicsImageRendererFormat *format = [[BMSDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    BMSDGraphicsImageRenderer *renderer = [[BMSDGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIColor *)bmsd_colorAtPoint:(CGPoint)point {
...
#if BMSD_UIKIT || BMSD_MAC
    if (self.CIImage) {
        imageRef = BMSDCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    CGImageRelease(imageRef);
    // Convert to color
    return BMSDGetColorFromPixel(pixel, bitmapInfo);
}
...
- (nullable NSArray<UIColor *> *)bmsd_colorsWithRect:(CGRect)rect {
    ...
    for (size_t index = start; index < end; index += 4) {
        ...
        UIColor *color = BMSDGetColorFromPixel(pixel, bitmapInfo);
...
- (nullable UIImage *)bmsd_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (CGImageGetBitsPerPixel(imageRef) != 32 ||
        ...
        BMSDGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
        [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
        imageRef = BMSDGraphicsGetImageFromCurrentImageContext().CGImage;
