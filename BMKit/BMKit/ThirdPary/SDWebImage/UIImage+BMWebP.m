...
@implementation UIImage (BMWebP)

- (NSInteger)bm_webpLoopCount
{
    NSNumber *value = objc_getAssociatedObject(self, @selector(bm_webpLoopCount));
    return value.integerValue;
}

+ (nullable UIImage *)bm_imageWithWebPData:(nullable NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self bm_rawWebpImageWithData:webpData];
    ...
    do {
        ...
        if (iter.blend_method == WEBP_MUX_BLEND) {
            image = [self bm_blendWebpImageWithCanvas:canvas iterator:iter];
        } else {
            image = [self bm_nonblendWebpImageWithCanvas:canvas iterator:iter];
        }
...
#if SD_UIKIT || SD_WATCH
    NSArray<UIImage *> *animatedImages = [self bm_animatedImagesWithImages:images durations:durations totalDuration:totalDuration];
    finalImage = [UIImage animatedImageWithImages:animatedImages duration:totalDuration / 1000.0];
    if (finalImage) {
        objc_setAssociatedObject(finalImage, @selector(bm_webpLoopCount), @(loopCount), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
#elif SD_MAC
...
+ (nullable UIImage *)bm_blendWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self bm_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
+ (nullable UIImage *)bm_nonblendWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self bm_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
+ (nullable UIImage *)bm_rawWebpImageWithData:(WebPData)webpData {
...
+ (NSArray<UIImage *> *)bm_animatedImagesWithImages:(NSArray<UIImage *> *)images durations:(int const * const)durations totalDuration:(NSTimeInterval)totalDuration
{
