...
- (void)setModel:(BMTZAssetModel *)model {
    ...
    int32_t imageRequestID = [[BMTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.bmtz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[BMTZImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(BMTZAssetCellType)type {
    _type = type;
    if (type == BMTZAssetCellTypePhoto || type == BMTZAssetCellTypeLivePhoto || (type == BMTZAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == BMTZAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.bmtz_left = self.videoImgView.bmtz_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == BMTZAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.bmtz_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView bm_showOscillatoryAnimationWithLayer:_selectImageView.layer type:BMTZOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[BMTZImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        BOOL iCloudSyncFailed = !imageData && [BMTZCommonTools isICloudSyncError:info[PHImageErrorKey]];
    ...
    if (_model.type == BMTZAssetCellTypeVideo) {
...
- (void)reload:(NSNotification *)noti {
    BMTZImagePickerController *tzImagePickerVc = (BMTZImagePickerController *)noti.object;
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage bmtz_imageNamedFromMyBundle:@"VideoSendIcon"]];
...
- (BMTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[BMTZProgressView alloc] init];
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.bmtz_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.bmtz_width - 27, 3, 24, 24);
    ...
    CGFloat progressXY = (self.bmtz_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.bmtz_height - 17, self.bmtz_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.bmtz_right, 0, self.bmtz_width - self.videoImgView.bmtz_right - 5, 17);
...
@interface BMTZAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation BMTZAlbumCell
...
- (void)setModel:(BMTZAlbumModel *)model {
    _model = model;
    ...
    [[BMTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.bmtz_width - 24, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.bmtz_height - titleHeight) / 2, self.bmtz_width - 80 - 50, titleHeight);
...
@implementation BMTZAssetCameraCell
