...
@implementation BMTZImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.bmtz_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[BMTZImageManager manager] fixOrientation:cropedImage];
...
@implementation UIImage (BMTZGif)

+ (UIImage *)bmsd_tz_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [BMTZImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
        ...
        for (size_t i = 0; i < count; i+=interval) {
            ...
            duration += [self bmsd_frameDurationAtIndex:i source:source] * MIN(interval, 3);
...
+ (float)bmsd_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
