...
@interface BMTZImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation BMTZImageManager

CGSize BMAssetGridThumbnailSize;
CGFloat BMTZScreenWidth;
CGFloat BMTZScreenScale;

static BMTZImageManager *manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (BMTZScreenWidth - 2 * margin - 4) / columnNumber - margin;
    BMAssetGridThumbnailSize = CGSizeMake(itemWH * BMTZScreenScale, itemWH * BMTZScreenScale);
}

- (void)configTZScreenWidth {
    BMTZScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    BMTZScreenScale = 2.0;
    if (BMTZScreenWidth > 700) {
        BMTZScreenScale = 1.5;
    }
}
...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(BMTZAlbumModel *model))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<BMTZAlbumModel *> *))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAllAlbumsWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<BMTZAlbumModel *> *))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<BMTZAssetModel *> *))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<BMTZAssetModel *> *))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        BMTZAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:config.allowPickingVideo allowPickingImage:config.allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(BMTZAssetModel *))completion {
    BMTZImagePickerConfig *config = [BMTZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index completion:(void (^)(BMTZAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (BMTZAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    BMTZAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == BMTZAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == BMTZAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == BMTZAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == BMTZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [BMTZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (BMTZAssetModelMediaType)getAssetType:(PHAsset *)asset {
    BMTZAssetModelMediaType type = BMTZAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != BMTZAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = BMTZScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < BMTZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = BMAssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth *BMTZScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(BMTZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[BMTZImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([BMTZImagePickerConfig sharedInstance].needFixComposition) {
...
- (BMTZAlbumModel *)modelWithResult:(PHFetchResult *)result collection:(PHAssetCollection *)collection isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets options:(PHFetchOptions *)options {
    BMTZAlbumModel *model = [[BMTZAlbumModel alloc] init];
...
- (BMTZAssetModel *)createModelWithAsset:(PHAsset *)asset {
    BMTZAssetModelMediaType type = [[BMTZImageManager manager] getAssetType:asset];
    NSString *timeLength = type == BMTZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[BMTZImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    BMTZAssetModel *model = [BMTZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
