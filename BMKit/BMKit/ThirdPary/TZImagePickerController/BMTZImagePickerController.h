...
- (void)imagePickerController:(BMTZImagePickerController *)picker didFinishPickingAndEditingVideo:(UIImage *)coverImage outputPath:(NSString *)outputPath error:(NSString *)errorMsg;

// When saving the edited video to the album fails, this callback will be called.
// 编辑后的视频自动保存到相册失败时，下面的代理方法会被执行
- (void)imagePickerController:(BMTZImagePickerController *)picker didFailToSaveEditedVideoWithError:(NSError *)error;
