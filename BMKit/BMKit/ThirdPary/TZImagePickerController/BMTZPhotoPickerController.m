...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[BMTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(BMTZAlbumModel *model) {
                self->_model = model;
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [BMTZCommonTools tz_isIPhoneX] ? 50 + (83 - 49) : 50;
