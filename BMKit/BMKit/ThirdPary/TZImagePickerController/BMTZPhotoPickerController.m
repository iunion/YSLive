...
@interface BMTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, PHPhotoLibraryChangeObserver> {
...
@property (nonatomic, strong) BMTZCollectionView *collectionView;
...
@implementation BMTZPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9.0, *)) {
            ...
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[BMTZImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    [BMTZCommonTools configBarButtonItem:cancelItem tzImagePickerVc:tzImagePickerVc];
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle bmtz_localizedStringForKey:@"Back"] style:UIBarButtonItemStylePlain target:nil action:nil];
        [BMTZCommonTools configBarButtonItem:backItem tzImagePickerVc:tzImagePickerVc];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[BMTZImageManager manager] getCameraRollAlbumWithFetchAssets:YES completion:^(BMTZAlbumModel *model) {
                self->_model = model;
                self->_models = [NSMutableArray arrayWithArray:self->_model.models];
                [self initSubviews];
            }];
        } else if (self->_showTakePhotoBtn || self->_isFirstAppear || !self.model.models || systemVersion >= 14.0) {
            [[BMTZImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<BMTZAssetModel *> *models) {
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        _collectionView = [[BMTZCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
        if (@available(iOS 13.0, *)) {
        ...
        [_collectionView registerClass:[BMTZAssetCell class] forCellWithReuseIdentifier:@"BMTZAssetCell"];
        [_collectionView registerClass:[BMTZAssetCameraCell class] forCellWithReuseIdentifier:@"BMTZAssetCameraCell"];
    } else {
        [_collectionView reloadData];
    }
    
    if (_showTakePhotoBtn) {
        _collectionView.contentSize = CGSizeMake(self.view.bmtz_width, ((_model.count + self.columnNumber) / self.columnNumber) * self.view.bmtz_width);
    } else {
        _collectionView.contentSize = CGSizeMake(self.view.bmtz_width, ((_model.count + self.columnNumber - 1) / self.columnNumber) * self.view.bmtz_width);
        if (_models.count == 0) {
            _noDataLabel = [UILabel new];
            _noDataLabel.textAlignment = NSTextAlignmentCenter;
            _noDataLabel.text = [NSBundle bmtz_localizedStringForKey:@"No Photos or Videos"];
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, [BMTZCommonTools tz_isRightToLeftLayout] ? 10 : -10, 0, 0);
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = 50 + [BMTZCommonTools tz_safeAreaInsets].bottom;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden && isFullScreen) top += [BMTZCommonTools tz_statusBarHeight];
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.bmtz_height - toolBarHeight - top : self.view.bmtz_height - top;;
    } else {
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.bmtz_height - toolBarHeight : self.view.bmtz_height;
    }
    _collectionView.frame = CGRectMake(0, top, self.view.bmtz_width, collectionViewHeight);
    _noDataLabel.frame = _collectionView.bounds;
    CGFloat itemWH = (self.view.bmtz_width - (self.columnNumber + 1) * itemMargin) / self.columnNumber;
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        toolBarTop = self.view.bmtz_height - toolBarHeight;
    } else {
        CGFloat navigationHeight = naviBarHeight + [BMTZCommonTools tz_statusBarHeight];
        toolBarTop = self.view.bmtz_height - toolBarHeight - navigationHeight;
    }
    _bottomToolBar.frame = CGRectMake(0, toolBarTop, self.view.bmtz_width, toolBarHeight);
    ...
    _previewButton.bmtz_width = !tzImagePickerVc.showSelectBtn ? 0 : previewWidth;
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
    ...
    _doneButton.frame = CGRectMake(self.view.bmtz_width - _doneButton.bmtz_width - 12, 0, _doneButton.bmtz_width, 50);
    _numberImageView.frame = CGRectMake(_doneButton.bmtz_left - 24 - 5, 13, 24, 24);
    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, self.view.bmtz_width, 1);
    
    [BMTZImageManager manager].columnNumber = [BMTZImageManager manager].columnNumber;
    [BMTZImageManager manager].photoWidth = tzImagePickerVc.photoWidth;
...
- (void)previewButtonClick {
    BMTZPhotoPreviewController *photoPreviewVc = [[BMTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.minImagesCount && tzImagePickerVc.selectedModels.count < tzImagePickerVc.minImagesCount) {
        NSString *title = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Select a minimum of %zd photos"], tzImagePickerVc.minImagesCount];
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            BMTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [BMTZImageManager manager].shouldFixOrientation = YES;
        __block UIAlertController *alertView;
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            BMTZAssetModel *model = tzImagePickerVc.selectedModels[i];
            BMTZImageRequestOperation *operation = [[BMTZImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    if (![BMTZImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[BMTZImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
                ...
                if (progress < 1 && havenotShowAlert && !alertView) {
                    alertView = [tzImagePickerVc showAlertWithTitle:[NSBundle bmtz_localizedStringForKey:@"Synchronizing photos from iCloud"]];
                    havenotShowAlert = NO;
                    return;
                }
                if (progress >= 1) {
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[BMTZImageManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        BMTZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZAssetCameraCell" forIndexPath:indexPath];
    ...
    BMTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        BMTZImagePickerController *tzImagePickerVc = (BMTZImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (BMTZAssetModel *model_item in selectedModels) {
            ...
            [UIView bm_showOscillatoryAnimationWithLayer:strongLayer type:BMTZOscillatoryAnimationToSmaller];
            if (strongCell.model.iCloudFailed) {
                NSString *title = [NSBundle bmtz_localizedStringForKey:@"iCloud sync failed"];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        } else {
            // 2. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                if ([[BMTZImageManager manager] isAssetCannotBeSelected:model.asset]) {
                    return;
                }
                if (!tzImagePickerVc.allowPreview) {
                    BOOL shouldDone = tzImagePickerVc.maxImagesCount == 1;
                    if (!tzImagePickerVc.allowPickingMultipleVideo && (model.type == BMTZAssetModelMediaTypeVideo || model.type == BMTZAssetModelMediaTypePhotoGif)) {
                        shouldDone = YES;
                    }
                    if (shouldDone) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Select a maximum of %zd photos"], tzImagePickerVc.maxImagesCount];
                [tzImagePickerVc showAlertWithTitle:title];
            }
        }
    };
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BMTZAssetModel *model = _models[index];
    if (model.type == BMTZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            BMTZImagePickerController *imagePickerVc = (BMTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle bmtz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            BMTZVideoPlayerController *videoPlayerVc = [[BMTZVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == BMTZAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            BMTZImagePickerController *imagePickerVc = (BMTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle bmtz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            BMTZGifPhotoPreviewController *gifPreviewVc = [[BMTZGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        UIAlertAction *cancelAct = [UIAlertAction actionWithTitle:[NSBundle bmtz_localizedStringForKey:@"Cancel"] style:UIAlertActionStyleCancel handler:nil];
        [alertController addAction:cancelAct];
        UIAlertAction *settingAct = [UIAlertAction actionWithTitle:[NSBundle bmtz_localizedStringForKey:@"Setting"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)pushPhotoPrevireViewController:(BMTZPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(BMTZPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[BMTZImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.bmtz_width <= 320) {
        return;
    }
    BMTZImagePickerController *imagePickerVc = (BMTZImagePickerController *)self.navigationController;
    [[BMTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    for (BMTZAssetModel *model in selectedModels) {
...
- (void)callDelegate:(PHAsset *)asset isSelect:(BOOL)isSelect {
    ...
    [[BMTZImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (isDegraded) return;
        __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            ...
            [[BMTZImageManager manager] savePhotoWithImage:photo meta:meta location:self.location completion:^(PHAsset *asset, NSError *error){
        ...
        if (videoUrl) {
            self.isSavingMedia = YES;
            [[BMTZImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                self.isSavingMedia = NO;
                if (!error && asset) {
                    [self addPHAsset:asset];
                } else {
...
- (void)addPHAsset:(PHAsset *)asset {
    BMTZAssetModel *assetModel = [[BMTZImageManager manager] createModelWithAsset:asset];
    BMTZImagePickerController *tzImagePickerVc = (BMTZImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == BMTZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            // 不能多选视频的情况下，不选中拍摄的视频
        } else {
            if ([[BMTZImageManager manager] isAssetCannotBeSelected:assetModel.asset]) {
                return;
            }
...
- (void)resetCachedAssets {
    [[BMTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[BMTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[BMTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            BMTZAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation BMTZCollectionView
