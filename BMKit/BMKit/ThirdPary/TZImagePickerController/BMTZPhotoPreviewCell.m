...
@implementation BMTZAssetPreviewCell
...
@implementation BMTZPhotoPreviewCell

- (void)configSubviews {
    self.previewView = [[BMTZPhotoPreviewView alloc] initWithFrame:CGRectZero];
...
@interface BMTZPhotoPreviewView ()<UIScrollViewDelegate>
@property (assign, nonatomic) BOOL isRequestingGIF;
@end

@implementation BMTZPhotoPreviewView
        ...
        [[BMTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            [[BMTZImageManager manager] getOriginalPhotoDataWithAsset:model.asset progressHandler:^(double progress, NSError *error, BOOL *stop, NSDictionary *info) {
                ...
                if (!isDegraded) {
                    self.isRequestingGIF = NO;
                    self.progressView.hidden = YES;
                    if ([BMTZImagePickerConfig sharedInstance].gifImagePlayBlock) {
                        [BMTZImagePickerConfig sharedInstance].gifImagePlayBlock(self, self.imageView, data, info);
                    } else {
                        self.imageView.image = [UIImage bmsd_tz_animatedGIFWithData:data];
                    }
                    [self resizeSubviews];
                }
            }];
...
- (void)resizeSubviews {
    _imageContainerView.bmtz_origin = CGPointZero;
    _imageContainerView.bmtz_width = self.scrollView.bmtz_width;
    ...
    if (image.size.height / image.size.width > self.bmtz_height / self.scrollView.bmtz_width) {
        ...
        height = floor(height);
        _imageContainerView.bmtz_height = height;
        _imageContainerView.bmtz_centerY = self.bmtz_height / 2;
    }
    if (_imageContainerView.bmtz_height > self.bmtz_height && _imageContainerView.bmtz_height - self.bmtz_height <= 1) {
        _imageContainerView.bmtz_height = self.bmtz_height;
    }
    CGFloat contentSizeH = MAX(_imageContainerView.bmtz_height, self.bmtz_height);
    _scrollView.contentSize = CGSizeMake(self.scrollView.bmtz_width, contentSizeH);
    [_scrollView scrollRectToVisible:self.bounds animated:NO];
    _scrollView.alwaysBounceVertical = _imageContainerView.bmtz_height <= self.bmtz_height ? NO : YES;
...
- (void)refreshScrollViewContentSize {
    if (_allowCrop) {
        ...
        CGFloat contentWidthAdd = self.scrollView.bmtz_width - CGRectGetMaxX(_cropRect);
        CGFloat contentHeightAdd = (MIN(_imageContainerView.bmtz_height, self.bmtz_height) - self.cropRect.size.height) / 2;
        CGFloat newSizeW = self.scrollView.contentSize.width + contentWidthAdd;
        CGFloat newSizeH = MAX(self.scrollView.contentSize.height, self.bmtz_height) + contentHeightAdd;
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _scrollView.frame = CGRectMake(10, 0, self.bmtz_width - 20, self.bmtz_height);
    static CGFloat progressWH = 40;
    CGFloat progressX = (self.bmtz_width - progressWH) / 2;
    CGFloat progressY = (self.bmtz_height - progressWH) / 2;
...
- (void)refreshImageContainerViewCenter {
    CGFloat offsetX = (_scrollView.bmtz_width > _scrollView.contentSize.width) ? ((_scrollView.bmtz_width - _scrollView.contentSize.width) * 0.5) : 0.0;
    CGFloat offsetY = (_scrollView.bmtz_height > _scrollView.contentSize.height) ? ((_scrollView.bmtz_height - _scrollView.contentSize.height) * 0.5) : 0.0;
...
@implementation BMTZVideoPreviewCell
...
@implementation BMTZGifPreviewCell
...
- (void)configPreviewView {
    _previewView = [[BMTZPhotoPreviewView alloc] initWithFrame:CGRectZero];
    __weak typeof(self) weakSelf = self;
