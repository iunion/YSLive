...
@interface BMTZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation BMTZPhotoPreviewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_currentIndex) {
        [_collectionView setContentOffset:CGPointMake((self.view.bmtz_width + 20) * self.currentIndex, 0) animated:NO];
    }
...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage bmtz_imageNamedFromMyBundle:@"navi_back"] forState:UIControlStateNormal];
...
- (void)configCollectionView {
    ...
    _collectionView.contentSize = CGSizeMake(self.models.count * (self.view.bmtz_width + 20), 0);
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop && _tzImagePickerVc.allowPickingImage) {
        ...
        [BMTZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = isFullScreen ? [BMTZCommonTools tz_statusBarHeight] : 0;
    CGFloat statusBarHeightInterval = isFullScreen ? (statusBarHeight - 20) : 0;
    CGFloat naviBarHeight = statusBarHeight + _tzImagePickerVc.navigationBar.bmtz_height;
    _naviBar.frame = CGRectMake(0, 0, self.view.bmtz_width, naviBarHeight);
    _backButton.frame = CGRectMake(10, 10 + statusBarHeightInterval, 44, 44);
    _selectButton.frame = CGRectMake(self.view.bmtz_width - 56, 10 + statusBarHeightInterval, 44, 44);
    _indexLabel.frame = _selectButton.frame;
    
    _layout.itemSize = CGSizeMake(self.view.bmtz_width + 20, self.view.bmtz_height);
    _layout.minimumInteritemSpacing = 0;
    _layout.minimumLineSpacing = 0;
    _collectionView.frame = CGRectMake(-10, 0, self.view.bmtz_width + 20, self.view.bmtz_height);
    ...
    _doneButton.frame = CGRectMake(self.view.bmtz_width - _doneButton.bmtz_width - 12, 0, _doneButton.bmtz_width, 44);
    _numberImageView.frame = CGRectMake(_doneButton.bmtz_left - 24 - 5, 10, 24, 24);
...
- (void)select:(UIButton *)selectButton {
    BMTZImagePickerController *_tzImagePickerVc = (BMTZImagePickerController *)self.navigationController;
    BMTZAssetModel *model = _models[self.currentIndex];
    if (!selectButton.isSelected) {
        // 1. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Select a maximum of %zd photos"], _tzImagePickerVc.maxImagesCount];
            ...
            if (model.type == BMTZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle bmtz_localizedStringForKey:@"Select the video when in multi state, we will handle the video as a photo"]];
            }
        }
    } else {
        ...
        for (BMTZAssetModel *model_item in selectedModels) {
            if ([model.asset.localIdentifier isEqualToString:model_item.asset.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    BMTZAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView bm_showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:BMTZOscillatoryAnimationToBigger];
    }
    [UIView bm_showOscillatoryAnimationWithLayer:_numberImageView.layer type:BMTZOscillatoryAnimationToSmaller];
}

- (void)backButtonClick {
    if (self.navigationController.childViewControllers.count < 2) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        if ([self.navigationController isKindOfClass: [BMTZImagePickerController class]]) {
            BMTZImagePickerController *nav = (BMTZImagePickerController *)self.navigationController;
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:[NSBundle bmtz_localizedStringForKey:@"Synchronizing photos from iCloud"]];
        return;
    }
    ...
    BMTZPhotoPreviewCell *cell = (BMTZPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    if (_tzImagePickerVc.allowCrop && [cell isKindOfClass:[BMTZPhotoPreviewCell class]]) { // 裁剪状态
        _doneButton.enabled = NO;
        [_tzImagePickerVc showProgressHUD];
        UIImage *cropedImage = [BMTZImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [BMTZImageCropManager circularClipImage:cropedImage];
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    offSetWidth = offSetWidth +  ((self.view.bmtz_width + 20) * 0.5);
    
    NSInteger currentIndex = offSetWidth / (self.view.bmtz_width + 20);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BMTZAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == BMTZAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZVideoPreviewCell" forIndexPath:indexPath];
        BMTZVideoPreviewCell *currentCell = (BMTZVideoPreviewCell *)cell;
    ...
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == BMTZAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZGifPreviewCell" forIndexPath:indexPath];
        BMTZGifPreviewCell *currentCell = (BMTZGifPreviewCell *)cell;
        currentCell.previewView.iCloudSyncFailedHandle = ^(id asset, BOOL isSyncFailed) {
            model.iCloudFailed = isSyncFailed;
            [weakSelf didICloudSyncStatusChanged:model];
        };
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZPhotoPreviewCell" forIndexPath:indexPath];
        BMTZPhotoPreviewCell *photoPreviewCell = (BMTZPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[BMTZPhotoPreviewCell class]]) {
        [(BMTZPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[BMTZPhotoPreviewCell class]]) {
        [(BMTZPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[BMTZVideoPreviewCell class]]) {
        BMTZVideoPreviewCell *videoCell = (BMTZVideoPreviewCell *)cell;
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == BMTZAssetModelMediaTypeVideo) {
    ...
    if (![[BMTZImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)didICloudSyncStatusChanged:(BMTZAssetModel *)model{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        BMTZAssetModel *currentModel = self.models[self.currentIndex];
...
- (void)showPhotoBytes {
    [[BMTZImageManager manager] getPhotosBytesWithArray:@[_models[self.currentIndex]] completion:^(NSString *totalBytes) {
        self->_originalPhotoLabel.text = [NSString stringWithFormat:@"(%@)",totalBytes];
    }];
}

- (NSInteger)currentIndex {
    return [BMTZCommonTools tz_isRightToLeftLayout] ? self.models.count - _currentIndex - 1 : _currentIndex;
}
