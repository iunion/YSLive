...
@interface BMTZVideoCropController ()<BMTZVideoEditViewDelegate,UICollectionViewDelegate, UICollectionViewDataSource> {
    AVPlayer *_player;
...
@property (strong, nonatomic) BMTZVideoEditView *videoEditView;
...
@implementation BMTZVideoCropController
...
- (void)configMoviePlayer {
    [[BMTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
    ...
    [[BMTZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configBottomToolBar {
    ...
    [_cancelButton setTitle:[NSBundle bmtz_localizedStringForKey:@"Cancel"] forState:0];
...
- (void)configVideoImageCollectionView {
    _itemW = (self.view.bmtz_width - VideoEditLeftMargin * 2 - 2 * PanImageWidth) / 10.0;
    ...
    [_collectionView registerClass:BMTZVideoPictureCell.class forCellWithReuseIdentifier:@"BMTZVideoPictureCell"];
    [self.view addSubview:_collectionView];
}

- (void)configVideoEditView {
    _videoEditView = BMTZVideoEditView.new;
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    if (self.imagePickerVc && [self.imagePickerVc isKindOfClass:[BMTZImagePickerController class]]) {
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarAndNaviBarHeight = statusBarHeight + self.navigationController.navigationBar.bmtz_height;
    ...
    CGFloat collectionViewH = (self.view.bmtz_width - VideoEditLeftMargin * 2 - 2 * PanImageWidth) / 10.0 * 2;
    _collectionView.frame = CGRectMake(0, self.view.bmtz_height - collectionViewH - toolBarHeight - statusBarHeight, self.view.bmtz_width, collectionViewH);
    _videoEditView.frame = _collectionView.frame;
    _cropVideoDurationLabel.frame = CGRectMake(0, _videoEditView.bmtz_bottom, self.view.bmtz_width, 20);
    
    CGFloat playerLayerHeight = CGRectGetMinY(_collectionView.frame) - statusBarHeight * 2;
    CGFloat playerLayerWidth = self.view.bmtz_width/self.view.bmtz_height * playerLayerHeight;
    CGFloat playerLayerLeft = (self.view.bmtz_width - playerLayerWidth) / 2.0;
    ...
    _playButton.frame = CGRectMake(0, statusBarAndNaviBarHeight, self.view.bmtz_width, playerLayerHeight - statusBarAndNaviBarHeight);
...
- (void)generateVideoImage {
    ...
    CGFloat maxCropWidth = self.view.bmtz_width - (VideoEditLeftMargin + PanImageWidth) * 2;
    ...
    if (durationSeconds <= self.imagePickerVc.maxCropVideoDuration) {
        ...
        _cropVideoDurationLabel.text = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Selected for %ld seconds"], (NSInteger)durationSeconds];
    } else {
        ...
        _cropVideoDurationLabel.text = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Selected for %ld seconds"],(long)self.imagePickerVc.maxCropVideoDuration];
    }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    BMTZVideoPictureCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BMTZVideoPictureCell" forIndexPath:indexPath];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX - _collectionViewBeginOffsetX >= self.view.bmtz_width) {
        [self.collectionView setContentOffset:CGPointMake(self.view.bmtz_width + _collectionViewBeginOffsetX, 0) animated:NO];
    } else if (_collectionViewBeginOffsetX - offsetX >= self.view.bmtz_width) {
        [self.collectionView setContentOffset:CGPointMake(_collectionViewBeginOffsetX - self.view.bmtz_width, 0) animated:NO];
    }
...
- (void)editViewCropRectBeginChange {
    ...
    _cropVideoDurationLabel.text = [NSString stringWithFormat:[NSBundle bmtz_localizedStringForKey:@"Selected for %ld seconds"], (NSInteger)second];
}
...
- (void)doneButtonClick {
    ...
    BMTZImagePickerController *imagePickerVc = self.imagePickerVc;
    [imagePickerVc showProgressHUD];
    [[BMTZImageManager manager] getVideoOutputPathWithAsset:_model.asset presetName:imagePickerVc.presetName timeRange:[self getCropVideoTimeRange] success:^(NSString *outputPath) {
...
- (void)callDelegateMethod {
    if (_outputPath) {
        ...
        if (self.imagePickerVc.saveEditedVideoToAlbum) {
            [[BMTZImageManager manager] saveVideoWithUrl:videoURL completion:^(PHAsset *asset, NSError *error) {
                if (error) { // 视频保存失败
        ...
        UIImage *coverImage = [[BMTZImageManager manager] getImageWithVideoURL:videoURL];
...
- (UIView *)iCloudErrorView{
    if (!_iCloudErrorView) {
        _iCloudErrorView = [[UIView alloc] initWithFrame:CGRectMake(0, [BMTZCommonTools tz_statusBarHeight] + 44 + 10, self.view.bmtz_width, 28)];
        UIImageView *icloud = [[UIImageView alloc] init];
        icloud.image = [UIImage bmtz_imageNamedFromMyBundle:@"iCloudError"];
        icloud.frame = CGRectMake(20, 0, 28, 28);
        [_iCloudErrorView addSubview:icloud];
        UILabel *label = [[UILabel alloc] init];
        label.frame = CGRectMake(53, 0, self.view.bmtz_width - 63, 28);
        label.font = [UIFont systemFontOfSize:10];
        label.textColor = [UIColor whiteColor];
        label.text = [NSBundle bmtz_localizedStringForKey:@"iCloud sync failed"];
...
@implementation BMTZVideoEditView {
...
- (void)layoutSubviews {
    _beginImgView.frame = CGRectMake(VideoEditLeftMargin, 0, PanImageWidth, self.bmtz_height);
    _indicatorLine.frame = CGRectMake(_beginImgView.bmtz_right - 2, 2, 2, self.bmtz_height - 4);
    _endImgView.frame = CGRectMake(self.bmtz_width - PanImageWidth - VideoEditLeftMargin, 0, PanImageWidth, self.bmtz_height);
    
    self.cropRect =  CGRectMake(VideoEditLeftMargin + PanImageWidth, 0, self.bmtz_width - VideoEditLeftMargin * 2 - PanImageWidth * 2, self.bmtz_height);
}
...
- (void)setCropRect:(CGRect)cropRect {
    _cropRect = cropRect;
    self.beginImgView.bmtz_left = cropRect.origin.x - PanImageWidth;
    self.indicatorLine.bmtz_left = cropRect.origin.x - self.indicatorLine.bmtz_width;
    self.endImgView.bmtz_left = CGRectGetMaxX(cropRect);
...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint bottomPoints[] = {
        CGPointMake(self.cropRect.origin.x, self.bmtz_height),
        CGPointMake(CGRectGetMaxX(self.cropRect), self.bmtz_height)
    };
...
- (void)indicatorLineAnimateWithDuration:(NSTimeInterval)duration cropRect:(CGRect)cropRect {
    ...
    [UIView animateWithDuration:duration delay:.0 options:UIViewAnimationOptionCurveLinear animations:^{
        self.indicatorLine.bmtz_left = CGRectGetMaxX(cropRect);
    } completion:nil];
}

- (void)resetIndicatorLine {
    [self.indicatorLine.layer removeAllAnimations];
    self.indicatorLine.bmtz_left = CGRectGetMinX(self.cropRect) - self.indicatorLine.bmtz_width;
}
...
- (void)panGestureAction:(UIGestureRecognizer *)gesture {
    ...
    switch (gesture.view.tag) {
        case 0: { // 左边拖拽
            CGFloat maxX = self.endImgView.bmtz_left - self.minCropRectWidth;
        ...
        case 1: { // 右边拖拽
            ...
            CGFloat  maxX = self.bmtz_width - VideoEditLeftMargin - PanImageWidth;
...
@implementation BMTZVideoPictureCell
