...
#import "YSRoomDelegate.h"
#import "YSRoomDefines.h"

@class YSRoomUser;
NS_ASSUME_NONNULL_BEGIN

@interface YSRoomInterface : NSObject
    ///-----------------------------------
    /// @name Properties
    ///-----------------------------------
...
/**
 ...
 @param appKey appID
 @param optional 房间扩展信息  YSRoomDefines.h文件中定义了相关传值key
...
/**
 ...
 @param level 日志等级
 @param logPath 日志需要写入沙盒的路径; 默认路径为：沙盒Libary/Caches/YSSDKLogs。
 @param debug 是否时debug模式，debug模式：控制台打印，release模式：控制台不打印。
 @return 0表示调用成功，非0表示调用失败
 */
+ (int)setLogLevel:(YSLogLevel)level logPath:(NSString * _Nullable)logPath debugToConsole:(BOOL)debug;

/**
 设置YSRoomInterfaceDelegate 代理
 @param roomDelegate 实现了YSRoomInterfaceDelegate回调接口的对象
 @return 0表示调用成功，非0表示调用失败
 */
- (int)registerRoomInterfaceDelegate:(id<YSRoomInterfaceDelegate> _Nullable)roomDelegate;

/**
 设置音视频数据 YSMediaFrameDelegate的代理
 
 @param mediaDelegate 实现了YSMediaFrameDelegate回调接口的对象
 @return 0表示调用成功，非0表示调用失败
 */
- (int)registerMediaDelegate:(id<YSMediaFrameDelegate> _Nullable)mediaDelegate;
...
/**
 ...
 @param port 服务器端口 若在初始化“- (int)initWithAppKey:optional:”接口中设置YSRoomSettingOptionalSecureSocket为YES，表示支持https或者wss，所以此端口需要设置为：443(默认)；若初始化接口设置为NO或者不设置，端口为：80(默认).
 @param nickname 本地用户的昵称
 @param roomParams Dic格式，内含进入房间所需的基本参数，比如：NSDictionary类型，key值详情见 YSRoomDefines.h 相关定义
...
/**
 ...
 @param tellWhom 要将此修改通知给谁。NSString类型，详情见 YSRoomDefines.h 相关定义. 可以是某一用户ID，表示此信令只发送给该用户
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (YSRoomUser * _Nullable)getRoomUserWithUId:(NSString *)peerId;
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setLocalVideoMirrorMode:(YSVideoMirrorMode)mode;
...
- (int)playVideo:(NSString *)peerID
      renderType:(YSRenderMode)renderType
...
/**
 ...
 @param toID 发送给谁 , NSString类型，详情见 YSRoomDefines.h 相关定义. 可以是某一用户ID，表示此信令只发送给该用户
...
/**
 ...
 @param toID 发布媒体流给谁 NSString类型，详情见 YSRoomDefines.h 相关定义. 可以是某一用户ID，表示此信令只发送给该用户
 @param attributes 参数
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setRemoteAudioVolume:(CGFloat)volume peerId:(NSString *)peerId type:(YSMediaType)type;
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (int)startServerRecord:(YSRecordType)recordType convert:(NSInteger)convert layout:(NSInteger)layout expiresabs:(NSInteger)expiresabs expires:(NSInteger)expires;
...
/**
 ...
 @param profile YSVideoProfile实例对象
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setVideoProfile:(YSVideoProfile *)profile;
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setSmallStreamParameter:(YSVideoProfile *)profile;

/**
 设置远端视频流默认类型，若不设置默认为YSVideoStream_Big

 @param streamType 视频流类型
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setRemoteDefaultVideoStreamType:(YSVideoStreamType)streamType;
...
/**
 ...
 @return 0表示调用成功，非0表示调用失败
 */
- (int)setRemoteVideoStreamType:(YSVideoStreamType)streamType peerId:(NSString *)peerID deviceId:(NSString * _Nullable)deviceID;
...
/**
 大规模房间时获取指定用户ID的用户
 在非大规模时使用- (YSRoomUser *)getRoomUserWithUId: 获取房间用户
 @param peerID 指定的用户ID
 @param callback 回调Block
 @return 0表示调用成功，非0表示调用失败
 */
- (int)getRoomUserWithPeerId:(NSString *)peerID callback:(void (^)(YSRoomUser *_Nullable user, NSError *_Nullable error))callback;
