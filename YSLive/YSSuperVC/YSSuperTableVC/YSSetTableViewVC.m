...
- (BOOL)verifyPhoneNum:(NSString *)phoneNum showMessage:(BOOL)showMessage
{
    if (![phoneNum bm_isNotEmpty])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:@"请输入手机号码" delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
    ...
    else if (![phoneNum bm_isValidChinesePhoneNumber])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:@"请输入正确的手机号码" delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
...
- (BOOL)verifyPassword:(NSString *)password showMessage:(BOOL)showMessage
{
    if (![password bm_isNotEmpty])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:@"请输入账户密码(8~16位)" delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
    ...
    else if (![password bm_isValidPassword])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:[NSString stringWithFormat:@"请输入%@-%@位字母+数字，字母区分大小写", @(YSPASSWORD_MINLENGTH), @(YSPASSWORD_MAXLENGTH)] delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
...
- (BOOL)verifyId:(NSString *)idNum showMessage:(BOOL)showMessage
{
    if (![idNum bm_isNotEmpty])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:@"请输入身份证号码" delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
    ...
    else if (![idNum bm_isValidChineseIDNumberString])
    {
        if (showMessage)
        {
            [self.progressHUD bm_showAnimated:YES withDetailText:@"请输入正确的身份证号码" delay:PROGRESSBOX_DEFAULT_HIDE_DELAY];
        }
